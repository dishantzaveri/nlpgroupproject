...
class Eval:
    ...
    def evaluate_rouge(self, predictions, core_json):
        ...
        for i, res in enumerate(predictions):
            ...
            # Extract the answer after the "assistant:" token
            answer = pred_ans.split("assistant:")[-1].strip()
            assert gt_ans != ''
            if answer == '':  # If the extracted answer is empty, set score to 0.
                score = 0
            else:
        ...
        return {
            ...
            'image_quantity_level-Result': {k: [sum(v), len(v)] for k, v in image_quantity_level_cnt.items()}
        }, eval_list
    ...
    def match_choice(self, text, option):
        ...
        def preprocess_option_string(option_string):
            ...
            # Escape any special regex characters in the processed option text
            special_chars = ["\\", ".", "^", "$", "*", "+", "?", "{", "}", "[", "]", "|", "(", ")"]
...
def main(args):
    ...
    if 'NeedleInAHaystack' in dataset or 'MMCoQA' in dataset:
        eval_result, eval_list = scorer.evaluate_needle(preds, core_annotation, needle='NeedleInAHaystack' in dataset)
